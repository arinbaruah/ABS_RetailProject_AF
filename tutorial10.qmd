...
Small significant lags can be ingnored. Make a generalisable non complex model. q = 0 again.
...
We cannot use AIC for ARIMA and ETS model. This is because ARIMA and ETS are looking at completely different data and accordingly forecasting. If the auto has the same number of D with the selected SARIMA models, can be compared.
...
fit %>% 
  select(auto,ets,stl) %>%
  forecast(h = "10 years") %>%
  autoplot(tail(aus_production),4 * 10,alpha = 0.5)
```
